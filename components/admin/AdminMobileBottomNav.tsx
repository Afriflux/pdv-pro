...
const PROFILE_LINKS: ProfileLink[] = [
  { icon: Building2, label: 'Yayyam ERP', href: '/admin/erp', color: 'text-emerald-500' },
  { icon: ShieldCheck, label: 'Vérifications KYC', href: '/admin/kyc', color: 'text-blue-500' },
  { icon: AlertTriangle, label: 'Plaintes', href: '/admin/complaints', color: 'text-orange-500' },
  { icon: History, label: 'Audit & Historique', href: '/admin/audit', color: 'text-purple-500' },
  { icon: Megaphone, label: 'Marketing', href: '/admin/marketing', color: 'text-pink-500' },
  { icon: MessageSquare, label: 'Chat WhatsApp', href: '/admin/whatsapp', color: 'text-green-500' },
  { icon: Puzzle, label: 'Intégrations', href: '/admin/integrations', color: 'text-indigo-500' },
  { icon: Zap, label: 'Maintenance', href: '/admin/maintenance', color: 'text-amber-500' },
  { icon: Globe, label: 'Marketplace', href: '/admin/marketplace', color: 'text-teal-500' },
  { icon: Settings, label: 'Mon Profil', href: '/admin/settings', color: 'text-gray-500' },
]

// ----------------------------------------------------------------
// Component
// ----------------------------------------------------------------
export function AdminMobileBottomNav({ adminName, adminRole, avatarUrl }: AdminMobileBottomNavProps) {
  ...
  const tabs = [
    { icon: LayoutDashboard, label: 'Accueil', href: '/admin' },
    { icon: UsersRound, label: 'Utilisateurs', href: '/admin/users' },
    { icon: ShoppingBag, label: 'Commandes', href: '/admin/orders' },
    { icon: Bell, label: 'Notifs', href: '/admin/notifications' },
  ]
  ...
  return (
    <>
      {/* ── Bottom Tab Bar ── */}
      <nav className="fixed bottom-0 left-0 right-0 z-50 lg:hidden pb-safe" role="navigation" aria-label="Navigation principale admin">
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 pt-2 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-gray-900 leading-tight">{adminName}</p>
                    <p className="text-[10px] text-amber-600 font-bold uppercase tracking-wider">{adminRole.replace('_', ' ')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <form action={signOut}>
