  ...
  return (
    <div className="fixed bottom-[85px] right-3 lg:bottom-6 lg:right-6 z-40 flex flex-col items-end gap-3 font-sans">
      ...
      {isOpen && (
        <div className="w-[340px] max-w-[calc(100vw-2rem)] bg-white rounded-3xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 zoom-in-95 duration-200">
          ...
          <div className="px-4 py-3 bg-white border-t border-gray-100 text-center">
            <span className="text-xs uppercase font-black tracking-widest text-gray-400">Temps de réponse ~5 min</span>
          </div>
        </div>
      )}

      {/* Tooltip (only when menu is closed, desktop only) */}
      {!isOpen && showTooltip && (
        <div className="hidden lg:block animate-in slide-in-from-right-4 fade-in duration-300 bg-white rounded-2xl shadow-xl border border-gray-100 p-4 max-w-[220px] relative">
          <button
      ...
      <button
        ...
        className={`group relative w-10 h-10 lg:w-14 lg:h-14 ${isOpen ? 'bg-gray-800' : 'bg-[#25D366]'} hover:opacity-90 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-105 active:scale-95`}
      >
        {/* Pulse ring — desktop only */}
        {!isOpen && <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-30 hidden lg:block" />}

        {isOpen ? (
          <X size={20} className="text-white relative z-10 lg:w-6 lg:h-6" />
        ) : (
           <svg viewBox="0 0 24 24" fill="white" className="w-5 h-5 lg:w-7 lg:h-7 relative z-10 drop-shadow-sm">
