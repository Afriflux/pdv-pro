...
export function ClosingView({ initialRequests }: { initialRequests: ClosingRequest[] }) {
  ...
  const getStatusBadge = (s: string) => {
    switch (s) {
      ...
      case 'CANCELLATION_REQUESTED': return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800">Demande d'Annulation</span>
                      ...
                      {req.status === 'PENDING' && (
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            disabled={loading === req.id}
                            className="p-2 text-gray-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-colors border border-transparent hover:border-purple-200"
                            title="Demander une Annulation"
                          >
                            <span className="text-xs font-medium truncate flex items-center gap-1"><XCircleIcon className="w-4 h-4" /> Annuler</span>
                          </button>
                          <button
