...
import { 
  Workflow, Plus, Play, Pause, Settings2, Trash2, 
  ArrowLeft, Save, Zap, MessageCircle, Mail, Globe, 
  Clock, GitMerge, CheckCircle2, ChevronRight, CheckSquare 
} from 'lucide-react'
import { toast } from 'sonner'

import { toggleWorkflowStatus, saveWorkflow, deleteWorkflow } from './actions'
...
export default function WorkflowsClient({ 
  ...
  const handleToggleStatus = async (id: string) => {
    ...
    const res = await toggleWorkflowStatus(id, wf.status)
    if (res.success) {
  ...
  const handleSave = async () => {
    if (!editingWorkflow || !storeId) return
    ...
    setIsSaving(true)
    const res = await saveWorkflow(editingWorkflow, storeId)
    setIsSaving(false)
                  ...
                  <div className="flex-1">
                    ...
                    >
                      <optgroup label="⚡ Actions (Communications)">
                        ...
                        <option value="telegram_vip">🎫 Lien Privé (Bot Telegram PDV Pro)</option>
