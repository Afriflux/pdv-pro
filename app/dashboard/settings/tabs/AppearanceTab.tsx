...
import { CheckCircle2, Loader2, Palette, Paintbrush, Image as ImageIcon, Trash2, Camera, DownloadCloud, Sparkles, Layout, ShoppingBag, Layers } from 'lucide-react'
...
const themeOptions = {
  storefront: [
    { id: 'classic', name: 'Classique / Glassmorphism', desc: 'Design épuré, fond clair, cartes de verre', icon: '🏪' },
    { id: 'cinematic', name: 'Cinématique Dark', desc: 'Style immersif sombre, hero fullscreen, hover reveal luxueux', icon: '🎬' },
    { id: 'cream_elegant', name: 'Premium Cream / Awa Mode', desc: 'Fonds crème, contrastes verts/or, ultra luxe', icon: '🌟' }
  ],
  funnel: [
    { id: 'classic', name: 'Split Layout Classique', desc: 'Image à gauche, détails à droite, CTA franc', icon: '📐' },
    { id: 'cinematic', name: 'Cinématique Dark', desc: 'Hero dramatique sombre, images en valeur maximale', icon: '🎥' },
    { id: 'cream_elegant', name: 'Premium Cream / Elevate', desc: 'Minimalisme chic, fond coquille d\'œuf, conversions optimisées Apple-like', icon: '💎' },
  ],
  productCard: [
    { id: 'minimal', name: 'Minimal', desc: 'Carte blanche épurée, zoom subtil au survol', icon: '🃏' },
    { id: 'hover_reveal', name: 'Hover Reveal', desc: 'Style sombre, overlay progressif, infos au survol', icon: '✨' },
    { id: 'bi_ton', name: 'Bi-Ton Luxe (Awa Mode)', desc: 'Moitié image carrée, moitié texte/prix aligné avec bouton acheter', icon: '🛍️' },
  ],
}
...
export function AppearanceTab({ store }: { store: Record<string, any> }) {
  ...
  // Theme preferences
  const [themeStorefront, setThemeStorefront] = useState(store?.theme_storefront ?? 'classic')
  const [themeFunnel, setThemeFunnel] = useState(store?.theme_funnel ?? 'classic')
  const [themeProductCard, setThemeProductCard] = useState(store?.theme_product_card ?? 'minimal')
  ...
  const saveThemes = async () => {
    ...
    try {
      await Actions.updateThemePreferences({
        theme_storefront: themeStorefront,
        theme_funnel: themeFunnel,
        theme_product_card: themeProductCard,
      })
      toast.success('Direction artistique sauvegardée !')
    } catch (err) {
      const error = err as Error
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-700 relative w-full xl:col-span-3 space-y-8">
      
      {/* =============================================
          SECTION 1 : IDENTITÉ VISUELLE (existante)
          ============================================= */}
      ...
      {/* =============================================
          SECTION 2 : DIRECTION ARTISTIQUE (NOUVEAU)
          ============================================= */}
      <div className="bg-white/80 backdrop-blur-xl border border-gray-200/60 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden relative">
        
        {/* Header */}
        <div className="bg-gradient-to-br from-[#0A1F1A] to-[#143D30] p-8 sm:p-12 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-emerald-500/10 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 text-white px-3 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase mb-4">
              <Sparkles size={14} /> Premium
            </div>
            <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Direction Artistique</h2>
            <p className="text-white/60 mt-2 text-sm max-w-xl">Choisissez le style visuel de votre boutique. Ces réglages impactent l&apos;affichage de votre vitrine, vos pages de vente et vos fiches produit.</p>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-12 space-y-10">
          ...
          {/* Thème Storefront */}
          <div>
            <h3 className="text-lg font-black text-gray-900 mb-1 flex items-center gap-2"><Layout size={20} className="text-emerald-600" /> Style de la Vitrine</h3>
            <p className="text-sm text-gray-500 mb-4">L&apos;apparence globale de votre page boutique publique.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {themeOptions.storefront.map((opt) => (
                <button
                  key={opt.id}
                  type="button"
                  onClick={() => setThemeStorefront(opt.id)}
                  className={`relative p-6 rounded-2xl border-2 text-left transition-all duration-300 hover:shadow-lg ${
                    themeStorefront === opt.id 
                ...
                >
                  ...
                  <h4 className="font-black text-gray-900 text-base">{opt.name}</h4>
          ...
          {/* Thème Funnel */}
          <div>
            <h3 className="text-lg font-black text-gray-900 mb-1 flex items-center gap-2"><Layers size={20} className="text-emerald-600" /> Style des Pages de Vente</h3>
            <p className="text-sm text-gray-500 mb-4">Le design de vos pages produit / tunnels de vente.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {themeOptions.funnel.map((opt) => (
                <button
                  key={opt.id}
                  type="button"
                  onClick={() => setThemeFunnel(opt.id)}
                  className={`relative p-6 rounded-2xl border-2 text-left transition-all duration-300 hover:shadow-lg ${
                    themeFunnel === opt.id 
                      ? 'border-emerald-500 bg-emerald-50/50 shadow-md ring-2 ring-emerald-500/20' 
                      : 'border-gray-200 bg-white hover:border-gray-300'
          ...
          {/* Thème Product Card */}
          <div>
            <h3 className="text-lg font-black text-gray-900 mb-1 flex items-center gap-2"><ShoppingBag size={20} className="text-emerald-600" /> Style des Fiches Produit</h3>
            <p className="text-sm text-gray-500 mb-4">L&apos;apparence des cartes produit dans votre vitrine et le marketplace.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {themeOptions.productCard.map((opt) => (
                <button
                  key={opt.id}
                  type="button"
                  onClick={() => setThemeProductCard(opt.id)}
                  className={`relative p-6 rounded-2xl border-2 text-left transition-all duration-300 hover:shadow-lg ${
                    themeProductCard === opt.id 
                      ? 'border-emerald-500 bg-emerald-50/50 shadow-md ring-2 ring-emerald-500/20' 
                      : 'border-gray-200 bg-white hover:border-gray-300'
                  }`}
                >
          ...
          {/* Save */}
          <div className="flex justify-end border-t border-gray-200/50 pt-8">
            <button 
              type="button"
              onClick={saveThemes}
              disabled={themeLoading}
              className="px-8 py-3.5 bg-[#0A1F1A] hover:bg-[#143D30] text-white rounded-2xl font-bold text-[15px] shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed w-full sm:w-auto hover:scale-[1.02]"
            >
              {themeLoading ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}
              Sauvegarder la Direction Artistique
            </button>
          </div>
        </div>
      </div>
