...
const DEFAULT_ROLES = [
  { name: 'Super Admin', color_cls: 'text-[#0F7A60] border-[#0F7A60]/30', bg_cls: 'bg-[#0F7A60]/10', is_custom: false, permissions: { dashboard: 'full', vendors: 'full', affiliates: 'full', clients: 'full', orders: 'full', closing: 'full', withdrawals: 'full', complaints: 'full', kyc: 'full', roles: 'full', settings: 'full', quotas: 'full', accounting: 'full', equity: 'full', maintenance: 'full', audit: 'full', apps: 'full', workflows: 'full', masterclass: 'full', themes: 'full' } },
  { name: 'Associé (Board)', color_cls: 'text-zinc-800 border-zinc-300', bg_cls: 'bg-zinc-100', is_custom: false, permissions: { dashboard: 'full', vendors: 'read', affiliates: 'read', clients: 'read', orders: 'read', closing: 'read', withdrawals: 'read', complaints: 'read', kyc: 'read', roles: 'read', settings: 'read', quotas: 'read', accounting: 'read', equity: 'read', maintenance: 'read', audit: 'read', apps: 'read', workflows: 'read', masterclass: 'read', themes: 'read' } },
  { name: 'Directeur Financier', color_cls: 'text-indigo-600 border-indigo-300', bg_cls: 'bg-indigo-50', is_custom: true, permissions: { dashboard: 'full', vendors: 'none', affiliates: 'read', clients: 'read', orders: 'full', closing: 'none', withdrawals: 'full', complaints: 'none', kyc: 'none', roles: 'none', settings: 'none', quotas: 'none', accounting: 'full', equity: 'read', maintenance: 'none', audit: 'none', apps: 'none', workflows: 'none', masterclass: 'none', themes: 'none' } },
  { name: 'Manager Vendeurs', color_cls: 'text-amber-600 border-amber-300', bg_cls: 'bg-amber-50', is_custom: true, permissions: { dashboard: 'read', vendors: 'full', affiliates: 'none', clients: 'read', orders: 'full', closing: 'none', withdrawals: 'none', complaints: 'full', kyc: 'full', roles: 'none', settings: 'none', quotas: 'none', accounting: 'none', equity: 'none', maintenance: 'none', audit: 'none', apps: 'none', workflows: 'none', masterclass: 'none', themes: 'none' } },
...
export default async function AdminRolesPage() {
  ...
  const { data: admins } = await supabaseAdmin
    .from('User')
    .select('id, email, name, role, created_at, internal_role_id')
    .in('role', ['super_admin', 'gestionnaire', 'support'])
    .order('created_at', { ascending: false })

  const adminList = (admins || []).map((u: Record<string, unknown>) => ({
    id: u.id as string,
